  ...
  async.parallel([
      ...
      function(callback){
          ...
          rest.get('http://search:8080/api/tracks/search?title=' + title + '&artist=' + artist).on('complete', function(data) {
              ...
              rest.get('http://images:8080/ImageService/api/covers/' + data["id"]).on('complete', function(data) {
              console.log('Cover image is ' + data["cover"]["url"]);
              callback(null, data["cover"]["url"]);
              });
          });
      }
  ],
